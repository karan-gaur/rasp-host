        ...
        if(err) {
            ...
            await bcrypt.compare(req.body.password, user.getHash()).then( (resolve) => {
                if(!resolve) {
                    ...
                    if(!(fs.existsSync(user.path.join(path.sep))&&fs.lstatSync(user.path.join(path.sep)).isDirectory)) {
                    ...
                    return res.json({"token": token, "path": [], "name": user.name, "admin": user.admin});
                }
            });
        } else {
            // No such user
            ...
            await bcrypt.compare(req.body.password, user.getHash()).then( (resolve) => {
                if(!resolve) {
                    ...
                    if(!user.admin) {
                        ...
                        return res.status(401).json({"error": "User does not have permission to create a new user"});
                    }
                    ...
                    new_user.storageLimit = req.body.storageLimit ? req.body.storageLimit*1024*1024*1024 : 
                        config.USER_STORAGE_LIMIT;
                    
                    if( fs.existsSync(new_user.path.join(path.sep)) && 
                    ...
                    bcrypt.hash(req.body.new_password, 10).then( function(hashPwd) {
                        ...
                        new_user.save( (saveError) => {
                            if(saveError && saveError.code === 11000) {
                                ...
                                logger.info(`Error saving new user - '${new_user.email}. Err - ${saveError}`);
                                return res.status(500).json({
                                    "error": "Internal server error. Contact System Administrator"
                                });
                            } 
...
router.post("/create_dummy", (req, res) => {
    ...
    new_user.storageLimit = req.body.storageLimit ? req.body.storageLimit*1024*1024*1024 : config.USER_STORAGE_LIMIT;
